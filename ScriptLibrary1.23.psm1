...
function Is-SoftwareInstalledWMI {
	...
	foreach ($Product in $Win32_Products) {
		...
		Write-Log -Message "  Name: ""$ProductName""    Version: ""$ProductVersion"""
	}
...
function Download-File {
	...
	do {
		try {
			...
			if ($RetryCount -gt 4) {
				Write-Log -Message "  $($MyInvocation.MyCommand.Name):: Could not download $Source after 5 tries - error: $_" -Type 3
				$StopLoop = $True
			}
			else {
				Write-Log -Message "  $($MyInvocation.MyCommand.Name):: Failed to download file from $Source - retrying" -Type 3
